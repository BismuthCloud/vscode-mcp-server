  ...
  try {
    ...
    if (!document) {
      try {
        ...
        logger.warn(
          `[getPreview] Could not read file: ${
    ...
    logger.warn(
      `[getPreview] Error getting preview: ${
        error instanceof Error ? error.message : String(error)
      }`
    );
  }
...
async function getLineText(
  ...
  try {
    ...
    logger.warn(
      `[getLineText] Error getting line text: ${
        error instanceof Error ? error.message : String(error)
      }`
    );
    return undefined;
...
export async function getSymbolHoverInfo(
  ...
  logger.info(
    `[getSymbolHoverInfo] Getting hover info for ${uri.toString()} at position (${
      position.line
    },${position.character})`
  );
  ...
  try {
    ...
    logger.info(
      `[getSymbolHoverInfo] Found ${commandResult.length} hover results`
    );

    // Map the hover results to a more friendly format
    const hovers = await Promise.all(
      commandResult.map(async (hover) => {
...
export async function searchWorkspaceSymbols(
  ...
  logger.info(
    `[searchWorkspaceSymbols] Starting with query: "${query}", maxResults: ${maxResults}`
  );
    ...
    const result = {
      symbols: limitedSymbols.map((symbol) => {
        const formatted = {
          ...
          location: `${uriToWorkspacePath(symbol.location.uri)}:${
...
export async function getDocumentSymbols(
  ...
  logger.info(
    `[getDocumentSymbols] Getting symbols for ${uri.toString()}, maxDepth: ${maxDepth}`
  );
  ...
  try {
    ...
    logger.info(
      `[getDocumentSymbols] Found ${symbols.length} top-level symbols`
    );
...
export function registerSymbolTools(server: McpServer): void {
  ...
  server.tool(
    ...
    async ({ query, maxResults = 10 }): Promise<CallToolResult> => {
      logger.info(
        `[search_symbols_code] Tool called with query="${query}", maxResults=${maxResults}`
      );
      ...
      try {
        ...
        logger.error(
          `[search_symbols_code] Error in tool: ${
            error instanceof Error ? error.message : String(error)
          }`
        );
        const errorMessage =
  ...
  server.tool(
        ...
        Requires exact symbol name and line number. If symbol not found on line, returns clear message.`,
    {
      ...
      symbol: z
        .string()
        .describe("The symbol name to look for on the specified line"),
    },
    async ({ path, line, symbol }): Promise<CallToolResult> => {
      logger.info(
        `[get_symbol_definition_code] Tool called with path="${path}", line=${line}, symbol="${symbol}"`
      );
      ...
      try {
        ...
        logger.error(
          `[get_symbol_definition_code] Error in tool: ${
            error instanceof Error ? error.message : String(error)
          }`
        );
        const errorMessage =
  ...
  server.tool(
        ...
        Shows classes, functions, methods, variables with line ranges. Use maxDepth for large files to avoid deep nesting.`,
    {
      path: z
        .string()
        .describe("The path to the file to analyze (relative to workspace)"),
      maxDepth: z
        .number()
        .optional()
        .describe("Maximum nesting depth to display (optional)"),
    },
    async ({ path, maxDepth }): Promise<CallToolResult> => {
      logger.info(
        `[get_document_symbols_code] Tool called with path="${path}", maxDepth=${maxDepth}`
      );
      ...
      try {
        ...
        logger.error(
          `[get_document_symbols_code] Error in tool: ${
            error instanceof Error ? error.message : String(error)
          }`
        );
        const errorMessage =
